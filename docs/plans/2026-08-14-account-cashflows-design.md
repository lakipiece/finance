...
> **⚠ 구현 완료 (2026-08-15). 최종 지표 체계는 [docs/portfolio-metrics.md](../portfolio-metrics.md)가 기준.**
> 구현 중 변경: ① 독립 페이지(`/portfolio/cashflows`) → **계좌 모달의 입출금 탭**으로 통합
> ② 수익 공식을 계좌별 하이브리드(원장 계좌 누적입금 | 미기록 계좌 매수원가)로 확정
> ③ `snapshots.account_breakdown` 추가로 과거 시점 하이브리드 계산 지원
