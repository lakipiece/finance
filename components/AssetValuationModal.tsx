    ...
    <div className={modal.overlayTop}>
      <div className={`${modal.containerLg}`} onClick={e => e.stopPropagation()}>
        ...
        <div className={modal.header}>
          <div>
            <h2 className="text-subhead font-medium text-ink">평가액 업데이트</h2>
            <p className="text-micro tracking-normal text-ink-4 mt-0.5">{assetName}</p>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-btn text-ink-5 hover:text-ink-2 hover:bg-surface-low transition-all" type="button">
        ...
        <div className={modal.body}>
          {/* 입력 폼 — 한 줄 */}
          <div className="bg-surface-low rounded-field px-3 py-2.5">
            <p className="text-micro font-medium text-ink-4 uppercase tracking-wide mb-2">
          ...
          {/* 기록 내역 */}
          <div>
            <p className="text-micro font-medium text-ink-4 uppercase tracking-wide mb-2">기록 내역</p>
            {histLoading ? (
              <div className="py-6 text-center text-body text-ink-5">로딩 중...</div>
            ) : history.length === 0 ? (
              <div className="py-6 text-center text-body text-ink-5">평가 기록이 없습니다</div>
            ) : (
                  ...
                  <div key={v.id}
                    ...
                    <div className="flex items-center gap-0.5 shrink-0">
                      <button onClick={() => startEdit(v)} title="수정"
                        className="p-1 rounded-cell text-ink-5 hover:text-ink hover:bg-surface-low transition-colors">
                      ...
                      <button onClick={() => handleDelete(v.id)} title="삭제"
                        className="p-1 rounded-cell text-ink-5 hover:text-danger hover:bg-danger/10 transition-colors">
                        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
