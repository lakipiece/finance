...
export default function PortfolioKpiCards({ summary }: Props) {
  ...
  const cards = [
    { label: '총 평가금액', value: formatWonRound(total_market_value), sub: '현재 시장가 기준', color: 'text-slate-800' },
    { label: '총 투자원금', value: formatWonRound(total_invested),      sub: '매수 원가 합계',  color: 'text-slate-800' },
    { label: '수익',        value: `${pnlPos ? '+' : ''}${formatWonRound(total_unrealized_pnl)}`, sub: '평가손익 합계', color: pnlColor },
    { label: '평가수익률',  value: pctStr(total_unrealized_pct), sub: '투자원금 대비',   color: pnlColor },
    { label: '누적 분배금', value: formatWonRound(total_dividends),      sub: '받은 배당·분배금', color: 'text-slate-800' },
  ]
