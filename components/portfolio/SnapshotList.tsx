          ...
          return (
            <div key={snap.id}
              ...
              <div className="flex items-start justify-between gap-2">
                <div>
                  <div className="flex items-center gap-1.5">
                    <p className="text-heading font-bold text-ink leading-tight">
                      {datePart.slice(0, 7)}
                    </p>
                  ...
                  <p className="text-micro tracking-normal text-ink-4 mt-0.5">{datePart}</p>
                </div>
                <div className="text-right shrink-0 flex flex-col items-end gap-0.5">
                  {mv != null ? (
                    <p className="text-heading font-bold text-ink-2 leading-tight tabular-nums">{fmtKrw(mv)}</p>
                  ) : (
              ...
              <div className="flex justify-between items-center mt-4 pt-2 border-t border-surface-low opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={e => handleExport(snap.id, e)}
                  className="flex items-center gap-1 px-1.5 py-1 rounded hover:bg-income/10 text-ink-5 hover:text-income transition-colors" title="CSV 내보내기">
                ...
                <button onClick={e => { e.stopPropagation(); router.push(`/portfolio/snapshots/${snap.id}`) }}
                  className="p-1.5 rounded hover:bg-surface-low text-ink-5 hover:text-ink-3 transition-colors" title="편집">
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                  </svg>
                </button>
                <button onClick={e => openClone(snap, e)}
                  className="p-1.5 rounded hover:bg-loss/10 text-ink-5 hover:text-loss transition-colors" title="복제">
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
                  </svg>
                </button>
                <button onClick={e => handleDelete(snap.id, e)}
                  className="p-1.5 rounded hover:bg-gain/10 text-ink-5 hover:text-gain transition-colors" title="삭제">
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {cloneTarget ? (
        <div className="modal-scrim fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="bg-surface-card rounded-card p-[13px] shadow-dialog max-w-sm w-full"
            ...
            <p className="text-subhead font-medium text-ink mb-1">스냅샷 복제</p>
