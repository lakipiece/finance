...
export interface ExpenseItemForEdit {
  id: number
  expense_date: string
  category: string
  detail: string
...
interface Props {
  editItem?: ExpenseItemForEdit | null
  onSaved: () => void
  onCancelEdit?: () => void
}
...
export default function ExpenseInputForm({ editItem, onSaved, onCancelEdit }: Props) {
  ...
  useEffect(() => {
    if (editItem) {
      setDate(editItem.expense_date)
      setMember(editItem.member || 'L')
      setCategory(editItem.category)
      setDetail(editItem.detail || '')
      setMethod(editItem.method || '')
      setAmount(String(editItem.amount))
      setMode('direct')
      setMemos([{ label: '', amount: '' }])
    } else {
      setDate(todayStr())
      setMember('L')
      setCategory('변동비')
  ...
  async function handleSave() {
    ...
    try {
      ...
      const url = editItem ? `/api/expenses/${editItem.id}` : '/api/expenses/create'
      const httpMethod = editItem ? 'PATCH' : 'POST'
        ...
        <div className="flex gap-1.5 mt-1">
          {['L', 'P'].map(m => (
            <button
              ...
              style={member === m ? { backgroundColor: palette.colors[0], borderColor: palette.colors[0] } : undefined}
            >
              {m}
            </button>
          ))}
      ...
      <div>
        ...
        <div className="flex gap-1.5 mt-1">
          <button
            ...
            style={mode === 'direct' ? { backgroundColor: palette.colors[0], borderColor: palette.colors[0] } : undefined}
          >
            직접 입력
          </button>
          <button
            ...
            style={mode === 'items' ? { backgroundColor: palette.colors[0], borderColor: palette.colors[0] } : undefined}
          >
            항목별 입력
          </button>
        </div>
        ...
        <div className="space-y-2">
          ...
          {memos.map((row, idx) => (
            <div key={idx} className="flex items-center gap-2">
              ...
              />
              {memos.length > 1 ? (
                <button type="button" onClick={() => removeMemoRow(idx)} className={btn.danger}>✕</button>
              ) : null}
            </div>
          ))}
          <button type="button" onClick={addMemoRow} className={btn.ghost}>+ 항목 추가</button>
          {totalFromMemos > 0 ? (
            <p className="text-xs text-slate-500 text-right">합계: {totalFromMemos.toLocaleString()}원</p>
          ) : null}
        </div>
      )}
